...
public class NativeUtil {
	...
	public static boolean compressBitmap(Bitmap bit, int quality, String fileName, boolean optimize) {
		...
		result = Bitmap.createBitmap(bit.getWidth() / 3, bit.getHeight() / 3, Config.ARGB_8888);// 缩小3倍
		...
		boolean saveResult = saveBitmap(result, quality, fileName, optimize);
		result.recycle();
		// } else {
	...
	public static boolean compressBitmap(Bitmap bit, int quality, int width, int height, String fileName, boolean optimize) {
		Log.d("native", "compress of native");
		Bitmap result = null;
	...
	private static boolean saveBitmap(Bitmap bit, int quality, String fileName, boolean optimize) {

		String result = compressBitmap(bit, bit.getWidth(), bit.getHeight(), quality, fileName.getBytes(), optimize);
		if ("1".equals(result)) {
			return true;
		} else {
			return false;
